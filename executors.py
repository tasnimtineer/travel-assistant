...
def summarize_daily_news(input_data: dict) -> str:
    topic = input_data.get("topic", "سياحة وسفر")
    api_key = os.environ.get("NEWS_API_KEY")

    if not api_key:
        return "لم يتم إعداد NEWS_API_KEY في ملف .env بعد. سجّل مجاناً في newsapi.org واحصل على مفتاح."

    try:
        response = requests.get(
            "https://newsapi.org/v2/everything",
            params={"q": topic, "language": "ar", "sortBy": "publishedAt", "pageSize": 5, "apiKey": api_key},
            timeout=10,
        )
        data = response.json()
        articles = data.get("articles", [])
        if not articles:
            return f"لم يتم العثور على أخبار حديثة عن '{topic}'."

        summary_lines = [f"- {a['title']}" for a in articles[:5]]
        return f"أهم أخبار '{topic}' اليوم:\n" + "\n".join(summary_lines)
    except Exception as e:
        return f"حدث خطأ أثناء جلب الأخبار: {e}"


# ---------------------------------------------------------------
# 4) البحث عن تذاكر الطيران - Amadeus API (بيئة تجريبية مجانية)
# ---------------------------------------------------------------
...
def search_flights(input_data: dict) -> str:
    ...
    if not token:
        return (
            "لم يتم إعداد AMADEUS_CLIENT_ID و AMADEUS_CLIENT_SECRET في .env بعد. "
            "سجّل مجاناً في developers.amadeus.com للحصول عليهما (بيئة تجريبية مجانية)."
        )
    ...
    try:
        ...
        if not offers:
            return f"لم يتم العثور على رحلات من {origin} إلى {destination} بتاريخ {date}."
        ...
        return f"حدث خطأ أثناء البحث عن الرحلات: {e}"
